...
// Manish Changes Aug-2023
const SqlDB = SQLite.openDatabase(
  {
    name: databaseName,
    location: 'default',
    // display_name: databaseDisplayName,
...
// ✅ UTILITY FUNCTION: Insert data with error tolerance - continues on individual row failures
    ...
    const insertNextItem = (index: number) => {
      ...
      tx.executeSql(
        ...
        (tx: any, results: any) => {
          successCount++;
          console.log(
            `✅ Inserted ${tableName} item ${index + 1}/${data.length}`,
          );
                  ...
                  if (index >= uses_log_data.length) {
                    ...
                    if (successCount === uses_log_data.length) {
                      ...
                      tx.executeSql(
                        ...
                        (tx, results) => {
                          console.log(
                            `✅ Successfully inserted ${successCount} uses_log records`,
                          );
                          resolve();
                        },
                        (tx, error) => {
                      ...
                      console.error(
                        `❌ Only ${successCount}/${uses_log_data.length} inserts succeeded. Restoring backup...`,
                      );
                      tx.executeSql(
                      ...
                      if (index >= settingData.length) {
                        ...
                        if (successCount === settingData.length) {
                          ...
                          tx.executeSql(
                            ...
                            (tx, results) => {
                              console.log(
                                `✅ Successfully inserted ${successCount} Settings records`,
                              );
                              resolve();
                            },
                            (tx, error) => {
                          ...
                          // Some failed - restore from backup
                          console.error(
                            `❌ Only ${successCount}/${settingData.length} inserts succeeded. Restoring backup...`,
                          );
                          tx.executeSql(
...
export async function insertPendingOrdersDiscount(PendingOrdersDiscount: any) {
  db1.transaction(tx => {
    tx.executeSql('DELETE from PendingOrdersDiscount', []).then(
      ([tx, results]) => {
        db1.transaction(tx => {
...
export async function insertPendingOrdersDetails(PendingOrdersDetails: any) {
  db1.transaction(tx => {
    tx.executeSql('DELETE from PendingOrdersDetails', []).then(
      ([tx, results]) => {
        db1.transaction(tx => {
...
export async function insertPendingOrdersMaster(PendingOrdersMaster: any) {
  db1.transaction(tx => {
    tx.executeSql('DELETE from PendingOrdersMaster', []).then(
      ([tx, results]) => {
        db1.transaction(tx => {
              ...
              tx.executeSql('DELETE from Sales', [], (tx, results) => {
                ...
                const insertNextItem = (index: number) => {
                  if (index >= SalesData.length) {
                    ...
                    console.log(
                      `📊 Sales insertion summary: ${successCount} successful, ${errorCount} failed`,
                    );

                    if (errorCount > 0) {
                      console.warn(
                        `⚠️ ${errorCount} Sales items failed but continuing with successful ones`,
                      );
                      console.warn('Failed items details:', errorDetails);
                    }
                    ...
                    tx.executeSql(
                      ...
                      (tx, results) => {
                        console.log(
                          `✅ Sales insertion completed: ${successCount}/${SalesData.length} records inserted`,
                        );
                        if (errorCount > 0) {
                          console.warn(
                            `⚠️ Note: ${errorCount} records were skipped due to errors`,
                          );
                        }
                        resolve();
                      },
                ...
                (tx, results) => {
                  ...
                  const insertNextItem = (index: number) => {
                    if (index >= SalesData.length) {
                      // All items processed - RESOLVE REGARDLESS OF ERRORS
                      console.log(
                        `📊 PaymentReceipt insertion summary: ${successCount} successful, ${errorCount} failed`,
                      );

                      if (errorCount > 0) {
                        console.warn(
                          `⚠️ ${errorCount} PaymentReceipt items failed but continuing with successful ones`,
                        );
                        console.warn('Failed items details:', errorDetails);
                      }
                      ...
                      tx.executeSql(
                        ...
                        (tx, results) => {
                          console.log(
                            `✅ PaymentReceipt insertion completed: ${successCount}/${SalesData.length} records inserted`,
                          );
                          if (errorCount > 0) {
                            console.warn(
                              `⚠️ Note: ${errorCount} records were skipped due to errors`,
                    ...
                    tx.executeSql(
                      ...
                      (tx, results) => {
                        successCount++;
                        console.log(
                          `✅ Inserted TX_PaymentReceipt_log item ${
                            index + 1
                          }/${SalesData.length}`,
                        );
                        insertNextItem(index + 1);
                ...
                (tx, results) => {
                  ...
                  const insertNextItem = (index: number) => {
                    if (index >= SalesData.length) {
                      // All items processed - RESOLVE REGARDLESS OF ERRORS
                      console.log(
                        `📊 Collections_log insertion summary: ${successCount} successful, ${errorCount} failed`,
                      );

                      if (errorCount > 0) {
                        console.warn(
                          `⚠️ ${errorCount} Collections_log items failed but continuing with successful ones`,
                        );
                        console.warn('Failed items details:', errorDetails);
                      }
                      ...
                      tx.executeSql(
                        ...
                        (tx, results) => {
                          console.log(
                            `✅ Collections_log insertion completed: ${successCount}/${SalesData.length} records inserted`,
                          );
                          if (errorCount > 0) {
                            console.warn(
                              `⚠️ Note: ${errorCount} records were skipped due to errors`,
                    ...
                    tx.executeSql(
                      ...
                      (tx, results) => {
                        successCount++;
                        console.log(
                          `✅ Inserted TX_Collections_log item ${index + 1}/${
                            SalesData.length
                          }`,
                        );
                        insertNextItem(index + 1);
                  ...
                  const insertNextItem = (index: number) => {
                    if (index >= OrderMaster.length) {
                      ...
                      if (successCount === OrderMaster.length) {
                        ...
                        tx.executeSql(
                          ...
                          (tx, results) => {
                            console.log(
                              `✅ Successfully inserted ${successCount} OrderMaster records`,
                            );
                            resolve();
                          },
                          (tx, error) => {
                        ...
                        // Some failed - restore from backup
                        console.error(
                          `❌ Only ${successCount}/${OrderMaster.length} inserts succeeded. Restoring backup...`,
                        );
                        tx.executeSql(
                          'DELETE FROM OrderMaster WHERE sync_flag="Y"',
                          [],
                    ...
                    tx.executeSql(
                      ...
                      [
                        ...
                        String(item.FromDate || ''),
                        String(item.ToDate || ''),
                        String(item.CollectionType ?? ''), // ✅ preserves 0
                        String(item.ExecutiveID || ''),
                        String(item.Remarks || ''),
                        '',
                        'Y',
                    ...
                    if (index >= OrderDetails.length) {
                      ...
                      if (successCount === OrderDetails.length) {
                        ...
                        tx.executeSql(
                          ...
                          (tx, results) => {
                            console.log(
                              `✅ Successfully inserted ${successCount} OrderDetails records`,
                            );
                            resolve();
                          },
                          (tx, error) => {
                        ...
                        // Some failed - restore from backup
                        console.error(
                          `❌ Only ${successCount}/${OrderDetails.length} inserts succeeded. Restoring backup...`,
                        );
                        tx.executeSql(
                          'DELETE FROM OrderDetails WHERE sync_flag="Y"',
                          [],
                  ...
                  if (index >= Resources.length) {
                    ...
                    if (successCount === Resources.length) {
                      ...
                      tx.executeSql(
                        ...
                        (tx, results) => {
                          console.log(
                            `✅ Successfully inserted ${successCount} Resources records`,
                          );
                          resolve();
                        },
                        (tx, error) => {
                      ...
                      // Some failed - restore from backup
                      console.error(
                        `❌ Only ${successCount}/${Resources.length} inserts succeeded. Restoring backup...`,
                      );
                      tx.executeSql(
...
export async function getOrderMasterSyncData(sync_flag: string) {
  //id,Current_date_time ,entity_type,entity_id ,latitude ,longitude ,total_amount ,from_date ,to_date ,collection_type ,user_id ,remark,selected_flag ,sync_flag ,check_date,DefaultDistributorId,ExpectedDeliveryDate
  let query =
...
// ✅ REMOVED: getOrderMasterByID function - not needed with proper batching strategy

// ✅ NEW FUNCTIONS: Reset sync flags back to "N" for failed syncs
...
export async function updateimageDetailSyncFlag(order_id: any) {
  // ❌ WRONG: Updates ALL images with same order_id
...
// ✅ CORRECT: Update individual image by its unique ID
    ...
    db1.transaction(tx => {
      ...
      tx.executeSql(
        ...
        (tx, results) => {
          ...
          tables.forEach(tbl => {
            console.log(
              '🔥performDeleteDbData DELETE: start',
              `DELETE FROM ${tbl};`,
            );
